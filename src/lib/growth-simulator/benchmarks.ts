...
export const FUNNEL_STAGE_BENCHMARKS: BenchmarkMetric[] = [
  ...
  {
    id: "kycVerificationRate",
    label: "KYC Verification Rate (approved → KYC verified)",
    unit: "pct",
    industry: "Finance — Lending",
    product: "Personal Loans / EMI / Credit Cards",
    geography: "India",
    p25: 72,
    median: 77,
    p75: 84,
    bestInClass: 92,
    tier: 5,
    source: "Fintel Connect 2025 Financial Services CPA Benchmarking Guide (decomposed) — RBI digital lending guidelines on mandatory KYC/document verification before disbursal",
    sourceDate: "2025-03-01",
    lastVerified: "2026-08-27",
    confidenceScore: 48,
    applicabilityScore: 65,
    derivation:
      "Fintel Connect's guide reports one combined approved-to-funded rate (52-70%, median 60%) — it doesn't separately break out KYC/document verification from the rest of that stage, even though RBI's digital lending guidelines make KYC a mandatory checkpoint before any disbursal. Rather than bolt an invented KYC-attrition number onto their real figure (which would silently shrink the overall approved→funded rate below what Fintel Connect actually reports), this splits their single number into two stages by an even geometric split — √0.60 ≈ 0.77 at the median — so KYC verification and disbursal each get their own visible rate, and multiplying them back together reproduces Fintel Connect's original combined figure exactly at every percentile.",
    notes: "Added Aug 2026 so KYC — routinely the single biggest post-approval bottleneck in India digital lending — shows up as its own line the Constraint Engine can actually flag, instead of being invisible inside a combined 'disbursal' number.",
  },
  {
    id: "disbursalRate",
    label: "Disbursal Rate (KYC verified → funded)",
    unit: "pct",
    industry: "Finance — Lending",
    product: "Personal Loans / EMI",
    geography: "India",
    ...
    bestInClass: 92,
    tier: 4,
    source: "Fintel Connect 2025 Financial Services CPA Benchmarking Guide (decomposed)",
    sourceDate: "2025-03-01",
    lastVerified: "2026-08-27",
    confidenceScore: 55,
    applicabilityScore: 65,
    derivation:
      "The other half of the same even geometric split described in kycVerificationRate's derivation above — this stage and that one multiply back to Fintel Connect's originally-reported 52-70% (median 60%) approved→funded rate. Kept at tier 4 rather than 5 since it's still anchored directly to Fintel Connect's real network data, just re-partitioned across two named stages instead of one.",
    notes: "Once KYC is verified, disbursal is largely an operational/documentation step, not a fresh credit decision — expect this to be higher than the KYC-verification rate itself in most lenders' data.",
  },
  ...
  {
    id: "investKycVerificationRate",
    label: "KYC Verification Rate (qualified → KYC verified)",
    unit: "pct",
    industry: "Finance — Investments",
    product: "Investments",
    geography: "India",
    ...
    applicabilityScore: 78,
    derivation:
      "RedSeer's fintech consumer research cites SEBI's own commentary on e-KYC/video-KYC completion rates for new investment-account applicants — India's KYC flow (identity verification, bank-account linking) has a well-documented drop-off point, read directly as this stage's conversion. Renamed Aug 2026 (was 'Account Opening Rate') — a completed KYC IS the account-opening event on most platforms, so the number is unchanged, but the label now says what the stage actually gates, since two more real steps (selecting a plan, funding it) sit downstream before money is actually invested.",
    notes: "KYC completion is usually the biggest drop-off point here — treat as a strong candidate constraint.",
  },
  {
    id: "investPlanSelectionRate",
    label: "Plan Selection Rate (KYC verified → plan selected)",
    unit: "pct",
    industry: "Finance — Investments",
    product: "Investments",
    geography: "India",
    ...
    bestInClass: 85,
    tier: 5,
    source: "RedSeer fintech consumer research on post-KYC platform engagement and first-action completion",
    sourceDate: "2025-06-01",
    lastVerified: "2026-08-27",
    confidenceScore: 38,
    applicabilityScore: 72,
    derivation:
      "RedSeer's post-onboarding engagement research (the same report used for the qualification-rate stage above) separately tracks what share of newly-KYC-verified users take a first meaningful in-app action — for an investment platform, that's browsing and selecting a specific fund/stock/SIP rather than leaving the account idle right after signup. Read directly as this stage's rate.",
    notes: "'Selected a plan' isn't money moved yet — a user can browse and pick a fund, then abandon before funding it. That's exactly what the next stage (Investment Completion Rate) is for.",
  },
  {
    id: "investCompletionRate",
    label: "Investment Completion Rate (plan selected → funded investment)",
    unit: "pct",
    industry: "Finance — Investments",
    product: "Investments",
    geography: "India",
    ...
    bestInClass: 90,
    tier: 5,
    source: "SEBI/NSDL/CDSL commentary on dormant demat accounts, cross-checked against RedSeer fintech consumer research on funding-step drop-off",
    sourceDate: "2025-06-01",
    lastVerified: "2026-08-27",
    confidenceScore: 36,
    applicabilityScore: 72,
    derivation:
      "SEBI and the depositories (NSDL/CDSL) have repeatedly flagged in public commentary how large a share of opened demat accounts never fund an actual trade — the 'dormant demat account' problem is well documented, not obscure. That figure describes qualified→invested overall, not this one narrow stage; RedSeer's engagement research on where exactly funding-step drop-off concentrates (bank-account linking, first-payment friction, cold feet on the actual transaction) was used to isolate this stage's rate specifically, rather than reusing the broader dormancy figure as if it applied to plan-selected users alone.",
    notes: "This is the real revenue event — an opened, even KYC-verified, unfunded account generates nothing. Added Aug 2026 so the funnel's endpoint is 'actually invested,' not 'account opened,' matching what the business actually gets paid on.",
  },
